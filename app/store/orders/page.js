...
function AllOrders() {
    const [allUserOrders, setUserOrders] = useState([]);
    ...
    useEffect(()=> {
        axiosInstance.get(`/api/ecom/v1/all/user/orders`).then((res)=> {
            // console.log(res);
            if (res.status === 200) {
                setUserOrders(res.data.all_user_orders);
                setLoading(false)
            }
