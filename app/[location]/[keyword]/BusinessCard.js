...
import MultiNumber from '@/components/users/Filter/MultiNumber';
...
function SearchKeyBusinessCard({ items, index }) {
    // const { user } = useAuth();
        ...
        <Row>
            ...
            <Col xs={24} sm={24} md={24} lg={0} xl={0} xxl={0}>
                ...
                {index === 4 ? <div className=' p-2'>
                    {items.category && <Enquiry1 id={items.category} getRequire={`best business in  of `} />}
                </div> : null}
            </Col>
        </Row>
    );
...
export default SearchKeyBusinessCard;
